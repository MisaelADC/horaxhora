<!DOCTYPE html>
  ...
  <body>
    ...
    height: 38px;
      ...
      <form class="row g-3 needs-validation" method="POST" action="/Wo" enctype="multipart/form-data">
        @csrf
        @method("POST")
        ...
        <select id="productSelect" class="form-control @error('Producto') is-invalid @enderror" name="Producto">
            ...
            @foreach ($Productos as $Pro)
                <option value="{{ $Pro->IdProduct }}" 
                    {{ (old('Producto') == $Pro->IdProduct || (isset($Wo) && $Wo->IdProduct == $Pro->IdProduct)) ? 'selected' : '' }}>
    ...
    <div class="col-md-3">
      ...
      <div class="col-md-6">
          ...
          <input type="text" class="form-control @error('Wo') is-invalid @enderror" id="validationCustom01" value="{{ old('Wo') }}" name="Wo" required>
          @error('Wo')
              <div class="invalid-feedback">
                  {{ $message }}
              </div>
      ...
      <div class="col-md-6">
        <label for="validationCustom01" class="form-label">Cantidad de Tapetes</label>
        <input type="Number" min="0" class="form-control @error('Cantidad') is-invalid @enderror" id="validationCustom01" value="{{ old('Cantidad') }}" name="Cantidad" required>
        @error('Cantidad')
            <div class="invalid-feedback">
                {{ $message }}
            </div>
          ...
          $('#productSelect').on('change', function() {
              ...
              if (productId) {
                  fetch(`/get-designs/${productId}`)
                      ...
                      .then(data => {
                          designSelect.empty().append('<option value="0">Seleccione un diseño</option>');
                  ...
                  designSelect.empty().append('<option value="0">Seleccione un diseño</option>').prop('disabled', true);
              }
          });
      });
