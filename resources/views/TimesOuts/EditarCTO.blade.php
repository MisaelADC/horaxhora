...
<body>
    ...
    <form class="row g-3 needs-validation" method="POST" action="/TimeOut/{{$CTO->IdTimeOut}}">
      @csrf
      @method("PUT")

      <div class="col-md-6">
          <label for="validationCustom01" class="form-label">Inicio</label>
          <input type="time" class="form-control @error('Inicio') is-invalid @enderror" id="validationCustom01" value="{{ $CTO->StartTime }}" name="Inicio" required>
          @error('Inicio')
      ...
      <div class="col-md-6">
          ...
          <input type="time" class="form-control @error('Final') is-invalid @enderror" id="validationCustom01" value="{{ $CTO->EndTime }}" name="Final" required>
          @error('Final')
              <div class="invalid-feedback">
                  {{ $message }}
              </div>
    ...
    <div class="col-md-12">
      ...
      <select class="form-control @error('Descripcion') is-invalid @enderror" id="Descripcion" name="Descripcion">
          @foreach ($Razones as $Raz)
              <option value="{{ $Raz->IdDowntimeReason}}" 
                  {{ (old('Descripcion') == $Raz->IdDowntimeReason || $CTO->downtimeReason->IdDowntimeReason == $Raz->IdDowntimeReason) ? 'selected' : '' }}>
                  {{ $Raz->Reason }}
              </option>
          @endforeach
      </select>
