...
<body>
    ...
    <a href="Turnos" class="NuevoTabla">Todos</a>
    @else
    <a href="Turnos/create" class="NuevoTabla">Nuevo</a>
    @endif
    ...
    <form action="{{ route('buscar') }}" method="POST">
      ...
      <div class="display">
       ...
       <div class="col-md-11">
        ...
        <input type="hidden" value="Turnos" id="validationCustomUsername" aria-describedby="inputGroupPrepend" method="POST" name="Tabla" accept="image/*"required>
    ...
    <table class="table" style="margin-top: 40px;">
      ...
      @foreach ($DatosPaginados as $Turnos)
      ...
      <tr>
        ...
        <td>{{ $Turnos["Shift"] }}</td>
        <td>{{ $Turnos["StartTime"] }}</td>
        <td>{{ $Turnos["EndTime"] }}</td>
    
        @if($Busqueda == 0)
        <td><a href="/Turnos/{{ $Turnos["IdShift"] }}/edit">Editar</a></td>
        <td><a href="/Turnos/{{ $Turnos["IdShift"] }}">Borrar</a></td> 
        @endif
        ...
        <form  class="row g-3 needs-validation" method="POST" action="/Elegir" enctype="multipart/form-data">
          ...
          <input type="text" value="{{$Turnos["IdShift"]}}" name="Id" hidden>
          <input type="text" value="{{$Turnos["Shift"]}}" name="Nombre" hidden>
          <input type="text" value="{{$R}}" name="R" hidden>
          <input type="text" value="Turnos" name="Tabla" hidden>
      ...
      <div id="pagination-links" class="pagination">
        ...
        <div>
            @if ($DatosPaginados->currentPage() > 1)
                <a href="{{ route('Turnos.index') }}?page={{ $DatosPaginados->currentPage() - 1 }}" class="btn btn-primary" style="margin-left: 7px">Anterior</a>
            @endif
    
            @if ($DatosPaginados->hasMorePages())
                <a href="{{ route('Turnos.index') }}?page={{ $DatosPaginados->currentPage() + 1 }}" class="btn btn-primary" style="margin-left: 7px">Siguiente</a>
            @endif
        </div>
    </div>
