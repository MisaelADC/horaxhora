...
<body>
    ...
    <h1>Razones (TimeOut)</h1>

    @if($BotonVerTodos == "Ver")
    <a href="Razones" class="NuevoTabla">Todos</a>
    @else
    <a href="Razones/create" class="NuevoTabla">Nuevo</a>
    @endif
    ...
    <form action="{{ route('buscar') }}" method="POST">
      ...
      <div class="display">
       ...
       <div class="col-md-11">
        ...
        <input type="hidden" value="Razones" name="Tabla" required>
    ...
    <table class="table" style="margin-top: 40px">
      ...
      <tr>
        ...
        <td>{{ $Razon["Tipo"] }}</td>        
        <td>{{ $Razon["replacement"] }}</td> 
      
        <td><a href="/Razones/{{ $Razon["IdDowntimeReason"] }}/edit">Editar</a></td>        
        <td><a href="/Razones/{{ $Razon["IdDowntimeReason"] }}">Borrar</a></td> 
      ...
      <div id="pagination-links" class="pagination">
        ...
        <div>
            @if ($DatosPaginados->currentPage() > 1)
                <a href="{{ route('Razones.index') }}?page={{ $DatosPaginados->currentPage() - 1 }}" class="btn btn-primary" style="margin-left: 7px">Anterior</a>
            @endif
    
            @if ($DatosPaginados->hasMorePages())
                <a href="{{ route('Razones.index') }}?page={{ $DatosPaginados->currentPage() + 1 }}" class="btn btn-primary" style="margin-left: 7px">Siguiente</a>
            @endif
        </div>
    </div>
