...
<body>
    ...
    <form class="row g-3 needs-validation" method="POST" action="/Razones" enctype="multipart/form-data">
      @csrf
      @method("POST")
    
    <div class="col-md-12">
        <label for="validationCustom01" class="form-label">Razon</label>
        <input type="text" class="form-control @error('Razon') is-invalid @enderror" id="validationCustom01" value="{{ old('Razon') }}" name="Razon" required>
        @error('Razon')
  ...
  <div class="col-md-6">
      ...
      <select class="form-control @error('Sustituto') is-invalid @enderror" id="Sustituto" name="Sustituto" required>
          <option value="">Seleccione...</option>
          <option value="Si" {{ old('Sustituto') == 'Si' ? 'selected' : '' }}>Sí</option>
          <option value="No" {{ old('Sustituto') == 'No' ? 'selected' : '' }}>No</option>
      </select>
      @error('Sustituto')
          <div class="invalid-feedback">
              {{ $message }}
