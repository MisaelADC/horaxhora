...
<body>
    ...
    <a href="Productos" class="NuevoTabla">Todos</a>
    @else
    <a href="Productos/create" class="NuevoTabla">Nuevo</a>
    @endif
    ...
    <form action="{{ route('buscar') }}" method="POST">
      ...
      <div class="display">
       ...
       <div class="col-md-7">
        ...
        <input type="hidden" value="Productos"   name="Tabla" required>
    ...
    <table class="table" style="margin-top: 40px">
      ...
      @foreach ($DatosPaginados as $Productos)
    
      {{-- @dd($Maquina) --}}

      <tr>
        <td scope="row">{{ ++$counter }}</td>
        <td>{{ $Productos["ItemCode"] }}</td>
        <td>{{ $Productos["Description"] }}</td>
        <td>{{ $Productos["Cycle"] }}</td>
        <td>{{ $Productos["Tipo"] }}</td>
        <td><a href="/Productos/{{ $Productos["IdProduct"] }}/edit">Editar</a></td>        
        <td><a href="/Productos/{{ $Productos["IdProduct"] }}">Borrar</a></td> 
        <td><a href="/Diseño/{{ $Productos["IdProduct"] }}">Diseños</a></td> 
      ...
      <div id="pagination-links" class="pagination">
        ...
        <div>
            @if ($DatosPaginados->currentPage() > 1)
                <a href="{{ route('Productos.index') }}?page={{ $DatosPaginados->currentPage() - 1 }}" class="btn btn-primary" style="margin-left: 7px">Anterior</a>
            @endif
    
            @if ($DatosPaginados->hasMorePages())
                <a href="{{ route('Productos.index') }}?page={{ $DatosPaginados->currentPage() + 1 }}" class="btn btn-primary" style="margin-left: 7px">Siguiente</a>
            @endif
        </div>
    </div>
