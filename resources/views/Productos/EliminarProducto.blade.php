...
<body>
    ...
    <form class="row g-3 needs-validation" novalidate method="POST" action="/Productos/{{$Producto->IdProduct}}">
      @csrf
      @method("DELETE") 

      <div class="col-md-6">
        <label for="validationCustom01" class="form-label">Codigo del Producto</label>
        <input type="text" class="form-control @error('Codigo') is-invalid @enderror" id="validationCustom01" value="{{$Producto->ItemCode}}" name="Codigo" readonly>
    </div>


    <div class="col-md-6">
      <label for="tipo" class="form-label">Tipo</label>
      <select class="form-control @error('Tipo') is-invalid @enderror" id="tipo" name="Tipo" readonly>
          <option value="Maquina" {{ $Producto->Tipo == 'Maquina' ? 'selected' : '' }}>Máquina</option>
          <option value="Mesa" {{ $Producto->Tipo == 'Mesa' ? 'selected' : '' }}>Mesa</option>
      </select>
      @error('Tipo')
    ...
    <div class="col-md-8">
        ...
        <input type="text" class="form-control" id="validationCustom01" value="{{$Producto->Description}}"  name="Descricion" readonly>
    </div>

    <div class="col-md-4">
      <label for="validationCustom01" class="form-label">Ciclo</label>
      <input type="text" class="form-control" id="validationCustom01" value="{{$Producto->Cycle}}" readonly>
  </div>
