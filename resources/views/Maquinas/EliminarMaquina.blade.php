...
<body>
    ...
    <div class="margen">
    <h1>Eliminar Máquina/Mesa</h1>
    </div>

    <form class="row g-3 needs-validation" novalidate method="POST" action="/Maquinas/{{$Maquina->IdMachine}}">
      @csrf
      @method("DELETE") 

      <div class="col-md-4">
        <label for="validationCustom01" class="form-label">Codigo de la Maquina</label>
        <input type="text" class="form-control @error('Codigo') is-invalid @enderror" id="validationCustom01" value="{{$Maquina->MachineCode}}" name="Codigo" readonly>
    </div>

    <div class="col-md-4">
        <label for="validationCustom01" class="form-label">Descripcion</label>
        <input type="text" class="form-control" id="validationCustom01" value="{{$Maquina->Description}}"  name="Descricion" readonly>
    </div>

    @if($Maquina->Tipo == "Maquina")
    <div class="col-md-4">
      <label for="validationCustom01" class="form-label">Tipo</label>
      <input type="text" class="form-control" id="validationCustom01" value="Máquina"  name="Descricion" readonly>
  </div>
  @else
  <div class="col-md-4">
      <label for="validationCustom01" class="form-label">Tipo</label>
      <input type="text" class="form-control" id="validationCustom01" value="Mesa"  name="Descricion" readonly>
  </div>
  @endif
