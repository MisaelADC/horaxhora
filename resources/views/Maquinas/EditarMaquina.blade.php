...
<body>
    ...
    <div class="margen">
    <h1>Editar Máquina/Mesa</h1>
    </div>

    <form class="row g-3 needs-validation" method="POST" action="/Maquinas/{{$Maquina->IdMachine}}" enctype="multipart/form-data" novalidate>
      @csrf
      @method("PUT")
  
      <div class="col-md-4">
        <label for="validationCustom01" class="form-label">Codigo de la Máquina/Mesa</label>
        <input type="text" class="form-control @error('Codigo') is-invalid @enderror" id="validationCustom01" value="{{$Maquina->MachineCode}}" name="Codigo" required>
        @error('Codigo')
    ...
    <div class="col-md-4">
        ...
        <input type="text" class="form-control" id="validationCustom01" value="{{$Maquina->Description}}"  name="Descripcion" required>
        @error('Descripcion')
        <div class="invalid-feedback">
            {{ $message }}
        </div>
    ...
    <div class="col-md-4">
      ...
      <select class="form-control" id="validationCustom01" name="Tipo" required>
          <option value="Maquina" {{ $Maquina->Tipo == 'Maquina' ? 'selected' : '' }}>Máquina</option>
          <option value="Mesa" {{ $Maquina->Tipo == 'Mesa' ? 'selected' : '' }}>Mesa</option>
      </select>
      @error('Tipo')
          <div class="invalid-feedback">
              {{ $message }}
