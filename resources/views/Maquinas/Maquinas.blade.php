...
<body>
    ...
    <h1>Máquinas/Mesas</h1>

 
    @if($BotonVerTodos == "Ver")
    <a href="Maquinas" class="NuevoTabla">Todos</a>
    @else
    <a href="Maquinas/create" class="NuevoTabla">Nuevo</a>
    @endif
    ...
    <form action="{{ route('buscar') }}" method="POST">
      ...
      <div class="display">
       ...
       <div class="col-md-7">
        ...
        <input type="hidden" value="Maquinas" id="validationCustomUsername" aria-describedby="inputGroupPrepend" method="POST" name="Tabla" accept="image/*"required>
    ...
    <table class="table" style="margin-top: 40px">
        <thead>
          ...
          <tbody>
            ...
            @php
            $counter = ($maquinasPaginadas->currentPage() - 1) * $maquinasPaginadas->perPage();
            @endphp

      @if(!empty($maquinasPaginadas))
      @foreach ($maquinasPaginadas as $Maquina)
    
      {{-- @dd($Maquina) --}}

      <tr>
        <td scope="row">{{ ++$counter }}</td>
        <td>{{ $Maquina["MachineCode"] }}</td>
        <td>{{ $Maquina["Description"] }}</td>
        @if($Maquina["Tipo"] == "Maquina")
        <td>Máquina</td>
  @else
  <td>{{ $Maquina["Tipo"] }}</td>
  @endif
        ...
        @if($Busqueda == 0)
        <td><a href="/Maquinas/{{ $Maquina["IdMachine"] }}/edit">Editar</a></td>
        <td><a href="/Maquinas/{{ $Maquina["IdMachine"] }}">Borrar</a></td> 
        <td><a href="/LimpiarMaquina/{{ $Maquina["IdMachine"] }}">Limpiar</a></td> 
        @endif

        @if($Busqueda == 1)
        <td>
        <form  class="row g-3 needs-validation" method="POST" action="/Elegir" enctype="multipart/form-data">
          @csrf
        
          <input type="text" value="{{$Maquina["IdMachine"]}}" name="Id" hidden>
          <input type="text" value="{{$Maquina["Description"]}}" name="Nombre" hidden>
          <input type="text" value="{{$R}}" name="R" hidden>
          <input type="text" value="Maquinas" name="Tabla" hidden>
      ...
      <div id="pagination-links" class="pagination">
        ...
        <p style="position: relative; top: 5px;">Página {{ $maquinasPaginadas->currentPage() }} de {{ $maquinasPaginadas->lastPage() }}</p>
    
        <!-- Botones para avanzar y retroceder -->
        <div style="margin-bottom: 30px;">
            @if ($maquinasPaginadas->currentPage() > 1)
                <a href="{{ route('Maquinas.index') }}?page={{ $maquinasPaginadas->currentPage() - 1 }}" class="btn btn-primary" style="margin-left: 7px">Anterior</a>
            @endif
    
            @if ($maquinasPaginadas->hasMorePages())
                <a href="{{ route('Maquinas.index') }}?page={{ $maquinasPaginadas->currentPage() + 1 }}" class="btn btn-primary" style="margin-left: 7px">Siguiente</a>
            @endif
        </div>
    </div>
