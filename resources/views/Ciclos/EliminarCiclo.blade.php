...
<body>
    ...
    <form class="row g-3 needs-validation" novalidate method="POST" action="/Ciclos/{{$Ciclo->IdCycle}}">
      @csrf
      @method("DELETE") 

      <div class="col-md-4">
        <label for="validationCustom01" class="form-label">Tiempo del ciclo</label>
        <input type="text" class="form-control @error('ciclo') is-invalid @enderror" id="validationCustom01" value="{{$Ciclo->duration}}" name="Ciclo" readonly>
    </div>

    <div class="col-md-4">
      <label for="validationCustom01" class="form-label">Producto</label>
      <input type="text" class="form-control @error('Producto') is-invalid @enderror" id="validationCustom01" value="{{$Ciclo->Producto}}-{{$Ciclo->ProductoD}}" name="Producto" readonly>
  </div>
  <div class="col-md-4">
    <label for="validationCustom01" class="form-label">Maquina</label>
    <input type="text" class="form-control @error("Maquina") is-invalid @enderror" id="validationCustom01" value="{{$Ciclo->Maquina}}-{{$Ciclo->MaquinaD }}" name="Maquina" readonly>
  </div>
