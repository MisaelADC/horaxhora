...
<body>
    ...
    @if($BotonVerTodos == "Ver")
      <a href="Usuarios" class="NuevoTabla">Todos</a>
    @else
    <a href="Usuarios/create" class="NuevoTabla">Nuevo</a>
    @endif
    ...
    <form action="{{ route('buscar') }}" method="POST">
      ...
      <div class="display">
       ...
       <div class="col-md-7">
        ...
        <input type="hidden" value="Usuarios" id="validationCustomUsername" aria-describedby="inputGroupPrepend" method="POST" name="Tabla" accept="image/*"required>
    ...
    <table class="table" style="margin-top: 40px;">
      ...
      @foreach ($DatosPaginados as $Usuario)

      <tr>
        <td scope="row">{{ ++$counter }}</td>
        <td>{{ $Usuario["CodeEmp"] }}</td>
        <td>{{ $Usuario["Name"] }}</td>
        <td>
          @switch($Usuario["Type"])
              @case(0)
        ...
        @if($Busqueda == 0)
        <td><a href="/Usuarios/{{ $Usuario["IdUser"] }}/edit">Editar</a></td>
        <td><a href="/Usuarios/{{ $Usuario["IdUser"] }}">Borrar</a></td> 
        @endif
        ...
        <form  class="row g-3 needs-validation" method="POST" action="/Elegir" enctype="multipart/form-data">
          ...
          <input type="text" value="{{$Usuario["IdUser"]}}" name="Id" hidden>
          <input type="text" value="{{$Usuario["Name"]}}" name="Nombre" hidden>
          <input type="text" value="{{$R}}" name="R" hidden>
          <input type="text" value="Usuarios" name="Tabla" hidden>
      ...
      <div id="pagination-links" class="pagination">
        ...
        <div>
            @if ($DatosPaginados->currentPage() > 1)
                <a href="{{ route('Usuarios.index') }}?page={{ $DatosPaginados->currentPage() - 1 }}" class="btn btn-primary" style="margin-left: 7px">Anterior</a>
            @endif
    
            @if ($DatosPaginados->hasMorePages())
                <a href="{{ route('Usuarios.index') }}?page={{ $DatosPaginados->currentPage() + 1 }}" class="btn btn-primary" style="margin-left: 7px">Siguiente</a>
            @endif
        </div>
    </div>
    @endif
    ...
    <div>
      @if ($DatosPaginados->currentPage() > 1)
          <a href="/E_Usuario?page={{ $DatosPaginados->currentPage() - 1 }}" class="btn btn-primary" style="margin-left: 7px">Anterior</a>
      @endif

      @if ($DatosPaginados->hasMorePages())
          <a href="/E_Usuario?page={{ $DatosPaginados->currentPage() + 1 }}" class="btn btn-primary" style="margin-left: 7px">Siguiente</a>
      @endif
  </div>
</div>
    @endif
