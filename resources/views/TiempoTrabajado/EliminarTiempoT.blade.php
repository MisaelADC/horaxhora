...
<head>
    ...
    <title>Eliminar Tiempo Trabajado</title>
...
<body>
    ...
    <div class="margen">
    <h1>Eliminar Tiempo Trabajado</h1>
    </div>

    <form class="row g-3 needs-validation" novalidate method="POST" action="/TiempoTrabajado/{{$TiempoT->IdTimeWorked}}">
      @csrf
      @method("DELETE") 
    ...
    <div class="col-md-6">
      ...
      <input type="text" class="form-control" id="validationCustom01" value="{{$TiempoT->user->CodeEmp." ".$TiempoT->user->Name}}"  name="Descricion" readonly>
    </div>

    @if(!empty($TiempoT->machine->MachineCode))
    <div class="col-md-6">
      <label for="validationCustom01" class="form-label">Maquina</label>
      <input type="text" class="form-control" id="validationCustom01" value="{{$TiempoT->machine->MachineCode. " " . $TiempoT->machine->Description}}"  name="Descricion" readonly>
    </div>
    @else
    <div class="col-md-6">
      <label for="validationCustom01" class="form-label">Maquina</label>
      <input type="text" class="form-control" id="validationCustom01" value="No Disponible"  name="Descricion" readonly>
    </div>
    @endif
