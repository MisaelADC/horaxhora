...
<head>
    ...
    <title>Editar Tiempo Trabajado</title>
...
<body>
  ...
  <div class="header" style="display: flex;">
    ...
    <form class="row g-3 needs-validation" method="POST" action="/TiempoTrabajado/{{$TiempoT->IdTimeWorked}}" enctype="multipart/form-data" novalidate>
    @csrf
    <div class="col-md-6">
      <label for="validationCustom01" class="form-label">Inicio</label>
      <input type="text" class="form-control @error('Codigo') is-invalid @enderror" id="validationCustom01" value="{{$TiempoT->HStart}}" name="Codigo" readonly>
  </div>

  <div class="col-md-6">
      <label for="validationCustom01" class="form-label">Final</label>
      <input type="text" class="form-control" id="validationCustom01" value="{{$TiempoT->HEnd}}"  name="Descricion" readonly>
  </div>

    </form>


    <center><h2 style="margin-top: 40px;">Cambiar a</h2></center>


    <form class="row g-3 needs-validation" method="POST" action="/TiempoTrabajado/{{$TiempoT->IdTimeWorked}}" enctype="multipart/form-data" >
      @csrf
      @method("PUT")
  ...
  <div class="col-md-4">
    <label for="validationCustom01" class="form-label">Inicio</label>
    <input type="time" class="form-control @error('Inicio') is-invalid @enderror" id="validationCustom01" value="{{ old('Inicio') }}" name="Inicio" required>
    @error('Codigo')
        <div class="invalid-feedback">
            {{ $message }}
        </div>
  ...
  <div class="col-md-4">
    <label for="validationCustom01" class="form-label">Final</label>
    <input type="time" class="form-control @error('Final') is-invalid @enderror" id="validationCustom01" value="{{ old('Final') }}" name="Final" required>
    @error('Descripcion')
        <div class="invalid-feedback">
            {{ $message }}
        </div>
    ...
    <select class="form-control @error('Maquina') is-invalid @enderror" id="Maquina" name="Maquina">
        @foreach ($Maquinas as $Maq)
            <option value="{{ $Maq->IdMachine }}" 
                {{ (old('Maquina') == $Maq->IdMachine || $TiempoT->IdMachine == $Maq->IdMachine) ? 'selected' : '' }}>
  ...
  <div class="col-md-6">
    ...
    <select class="form-control @error('Usuario') is-invalid @enderror" id="Usuario" name="Usuario">
        @foreach ($Usuarios as $Usu)
            <option value="{{ $Usu->IdUser }}" 
                {{ (old('Usuario') == $Usu->IdUser || $TiempoT->IdUser == $Usu->IdUser) ? 'selected' : '' }}>
